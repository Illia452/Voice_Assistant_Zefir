...
class UI_MainWindow(QMainWindow):
    ...
    def clickButton_Setting(self):
        self.setting_anim.stop()

        if self.status_buttonSetting:
            self.setting_window.show()
            self.setting_window.raise_()

            self.setting_anim.setStartValue(self.setting_closed_rect)
            self.setting_anim.setEndValue(self.setting_open_rect)

            self.status_buttonSetting = False
        else:
            self.setting_anim.setStartValue(self.setting_window.geometry())
            self.setting_anim.setEndValue(self.setting_closed_rect)

            self.setting_anim.finished.connect(self.setting_window.hide)

            self.status_buttonSetting = True

        self.setting_anim.start()
    ...
    def createSettingWindoww(self):
        ...
        self.setting_closed_rect = QtCore.QRect(20, 30, 40, 40)
        self.setting_open_rect = QtCore.QRect(30, 120, 220, 450)

        self.setting_window.setGeometry(self.setting_closed_rect)
        ...
        self.setting_anim = QtCore.QPropertyAnimation(
            self.setting_window, b"geometry"
        )
        self.setting_anim.setDuration(320)
        self.setting_anim.setEasingCurve(QtCore.QEasingCurve.OutBack)
    ...
    def toggleSettingsPanel(self, event=None):
        ...
        if not self.settings_expanded:
            anim.setStartValue(QRect(20, 30, 40, 40))
            anim.setEndValue(QRect(30, 120, 220, 450))
            ...
            anim.setStartValue(QRect(30, 120, 220, 450))
            anim.setEndValue(QRect(20, 30, 40, 40))
            self.settings_panel.setStyleSheet("""
                QWidget {
                    background-color: rgb(209, 213, 219);
                    border-radius: 8px;
